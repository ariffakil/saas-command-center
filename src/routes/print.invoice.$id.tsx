...
import { getInvoiceView } from "@/lib/document-data";
import { Printer, ArrowLeft } from "lucide-react";

export const Route = createFileRoute("/print/invoice/$id")({
  component: PrintInvoice,
  loader: ({ params }) => {
    const doc = getInvoiceView(params.id);
  ...
  notFoundComponent: () => (
    <div className="p-10 text-center">
      ...
      <Link to="/invoices" className="mt-2 inline-block text-primary hover:underline">Back</Link>
    </div>
  ),
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "16px 0" }}>
      <div className="no-print" style={{ maxWidth: 820, margin: "0 auto 12px", display:"flex", justifyContent:"space-between", padding: "0 8px" }}>
        <Link to="/invoices" style={{ display:"inline-flex", alignItems:"center", gap:6, fontSize:14, color:"#374151", textDecoration:"none" }}>
