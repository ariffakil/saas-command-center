...
function makeIcon(online: number, offline: number, inactive: boolean) {
  ...
  const html = `
    <div style="position:relative;display:flex;align-items:center;justify-content:center;">
      ...
      ${!inactive ? `<span style="position:absolute;top:-2px;right:-2px;width:12px;height:12px;border-radius:50%;background:${color};border:2px solid white;animation:bpulse 1.6s ease-out infinite;"></span>` : ""}
    </div>
    <style>@keyframes bpulse{0%{transform:scale(1);opacity:.9}100%{transform:scale(2.4);opacity:0}}</style>
  `;
...
export function BranchMap({ subscriberFilter = "all" }: { subscriberFilter?: string }) {
  ...
  if (!mounted) {
    return <div className="h-[560px] w-full animate-pulse rounded-xl bg-muted" />;
  }
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border">
      <MapContainer
        ...
        zoom={3}
        scrollWheelZoom
        style={{ height: 560, width: "100%", background: "hsl(var(--muted))" }}
      >
        <TileLayer
